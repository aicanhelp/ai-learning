...
from electrax.finetune import preprocessing
from electrax.finetune import task_builder
    ...
    def evaluate_task(self, task, split="dev", return_results=True):
        ...
        for r in results:
            if r["task_id"] != len(self._tasks):  # ignore padding examples
                r = tfutils.nest_dict(r, self._config.task_names)
    ...
    def write_classification_outputs(self, tasks, trial, split):
        ...
        for task_name in logits:
            ...
            if trial <= self._config.n_writes_test:
                tfutils.write_pickle(logits[task_name], self._config.test_predictions(
...
def write_results(config: FinetuningConfig, results):
    ...
    tfutils.mkdir(config.results_txt.rsplit("/", 1)[0])
    tfutils.write_pickle(results, config.results_pkl)
...
def run_finetuning(config: FinetuningConfig):
    """Run finetuning."""
    ...
    heading = lambda msg: tfutils.heading(msg + ": " + heading_info)
    heading("Config")
    tfutils.log_config(config)
    ...
    while config.num_trials < 0 or trial <= config.num_trials:
        ...
        if config.do_train:
            tfutils.rmkdir(config.model_dir)
            ...
            if config.write_test_outputs and trial <= config.n_writes_test:
                ...
                for task in tasks:
                    ...
                    if task.name in ["cola", "mrpc", "mnli", "sst", "rte", "qnli", "qqp",
                        ...
                        preds = tfutils.load_json(config.qa_preds_file(task.name + "_dev"))
                        null_odds = tfutils.load_json(config.qa_na_file(task.name + "_dev"))
                        for q, _ in preds.items():
                            if null_odds[q] > config.qa_na_threshold:
                                preds[q] = ""
                        tfutils.write_json(preds, config.test_predictions(
                            task.name, "dev", trial))
                        ...
                        if config.num_trials > 1:
                            tfutils.write_json(preds, config.qa_preds_file(task.name + "_dev_" + str(trial)))
                    else:
            ...
            if config.write_test_outputs and trial <= config.n_writes_test:
                ...
                for task in tasks:
                    ...
                    if task.name in ["cola", "mrpc", "mnli", "sst", "rte", "qnli", "qqp",
                        ...
                        scorer.write_predictions()
                        preds = tfutils.load_json(config.qa_preds_file(task.name + "_eval"))
                        null_odds = tfutils.load_json(config.qa_na_file(task.name + "_eval"))
                        for q, _ in preds.items():
                            if null_odds[q] > config.qa_na_threshold:
                                preds[q] = ""
                        tfutils.write_json(preds, config.test_predictions(
                        ...
                        if config.num_trials > 1:
                            tfutils.write_json(preds, config.qa_preds_file(task.name + "_eval_" + str(trial)))
                    else:
                        utils.log("Skipping task", task.name,
                                  "- writing predictions is not supported for this task")
            else:
...
def main():
    ...
    run_finetuning(FinetuningConfig(
